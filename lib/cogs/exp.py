...
class Exp(Cog):
    ...
    async def display_rank(self, ctx, target: Optional[Member]):
        ...
        try:
            await ctx.send(f'{target.display_name} is rank {ids.index(target.id) + 1} of {len(ids)}', delete_after=10)
        except ValueError:
            await ctx.send('That member is not tracked by the experience system', delete_after=10)
